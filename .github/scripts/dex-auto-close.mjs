...
const failures = [];
for (const id of ids) {
  // Skip if already complete (idempotent — re-runs are safe).
  let alreadyDone = false;
  try {
    const out = execSync(`dex show ${id} --json`, { encoding: "utf8" });
    ...
    console.warn(`  [${id}] dex show failed (task may not exist): ${err.message}`);
