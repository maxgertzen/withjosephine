...
test.describe("Cancel-scheduled — purchaser terminates a scheduled gift (D-11) — mock mode", () => {
  test("arms via 5s confirm, posts to /cancel-scheduled, intercepted payload + 200 round-trip", async ({
    page,
    request,
  }) => {
    test.setTimeout(2 * 60 * 1000);

    const purchaserEmail = "cancel-scheduled-purchaser@withjosephine.com";
    ...
    await page.locator("#gift-purchaser-first-name").fill("CancelPurchaser");
    await page.locator("#gift-purchaser-email").fill(purchaserEmail);
    await page.locator("#gift-recipient-name").fill("CancelRecipient");
    await page
      .locator("#gift-recipient-email")
      .fill("cancel-scheduled-recipient@withjosephine.com");
    const initialSendAt = await datetimeLocalPlus(page, 60 * 24);
    await page.locator("#gift-send-at").fill(initialSendAt);
    await page.locator("#gift-message").fill("D-11 cancel-scheduled e2e walk.");
    ...
    const cancelCta = page.getByRole("button", { name: /^cancel this gift$/i });
    await expect(cancelCta).toBeVisible();
    ...
    await page.route(
      `**/api/gifts/${submissionId}/cancel-scheduled`,
      async (route) => {
