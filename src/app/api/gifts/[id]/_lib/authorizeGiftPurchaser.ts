 ...
 *
 * Timing parity: the 401 paths (no cookie / no active session) issue a
 * `findSubmissionById` lookup before returning 401, so a remote attacker
 * can't distinguish "you're unauthenticated" from "your session was valid
 * but the row doesn't exist or isn't yours" by response time alone.
 */
