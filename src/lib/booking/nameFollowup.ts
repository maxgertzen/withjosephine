...
export function nameFollowupKey(optionValue: string): string {
  return `${NAME_FOLLOWUP_PREFIX}${optionValue}`;
}

export function isNameFollowupKey(key: string): boolean {
  return key.startsWith(NAME_FOLLOWUP_PREFIX);
}

export function nameFollowupOptionValue(key: string): string {
  return key.slice(NAME_FOLLOWUP_PREFIX.length);
}

export function isNameFollowupEnabled(option: SanityFormFieldOption | undefined): boolean {
  return option?.nameFollowup?.enabled === true;
}

export function statusLineFor(count: number, target: number): string {
...
function capitalSpelledOut(n: number): string {
