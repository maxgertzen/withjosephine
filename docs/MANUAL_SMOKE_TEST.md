...
| Test inbox | `hello+staging@withjosephine.com` (forwarder — confirm with the maintainer) |
| Stripe test card | `4242 4242 4242 4242` · any future expiry · any 3-digit CVC · any ZIP |
| Admin token | Configured admin-token store (e.g. password manager entry "Josephine — Admin API Key (staging)") |

Use **incognito / private windows** for every customer journey. Cached state breaks the test.

For email addresses across flows, use **Gmail plus-addressing**: `hello+test1@…`, `hello+test2@…`, `hello+gift-recipient@…`. All land in the same inbox; the site treats each plus-address as a different user.
...
3. Tick the consent boxes, enter name + `hello+test1@withjosephine.com`, continue.
...
3. Fill purchaser name + `hello+test2@withjosephine.com`. Tick the 2 consents (Art. 6 + cooling-off — no Art. 9 on the purchaser side).
...
3. Enter recipient name + `hello+gift-recipient@withjosephine.com`. Tick the 3 consents (Art. 6 + Art. 9 + cooling-off — recipient acks Art. 9 here).
...
- ✅ **Order Confirmation**-style email arrives in `hello+gift-recipient`.
...
3. Fill purchaser name + `hello+test3@withjosephine.com`.
4. Fill recipient name + `hello+gift-recipient-2@withjosephine.com`.
...
2. Enter `hello+test3@withjosephine.com`. **Send sign-in link**.
...
| **Regenerate claim link** | Click "Regenerate" → confirm + enter admin reason (test) | Old link invalidated; new link displayed |
...
2. Listen page asks to confirm the email. Enter `hello+test1@withjosephine.com`.
...
- ✅ **Contact Message** email arrives in `hello@withjosephine.com` (the real inbox, not the test forwarder).
...
## Cleanup — DELETE TEST DATA when finished

Once every journey passes, the maintainer runs cleanup. The list:

| Surface | What to clear |
|---------|---------------|
| **D1 staging** | Delete every row from `submissions`, `gift_purchases`, `auth_sessions`, `listen_magic_link`, `listen_audit` where the email matches `hello+*@withjosephine.com` |
| **Sanity staging dataset** | Delete every `bookingSubmission`, `giftPurchase`, `magicLinkRequest` doc created during the test |
| **R2 staging bucket** | Delete every photo upload key written during the test (look at `withjosephine-booking-photos-staging` for keys timestamped during the test window) |
| **Sanity Studio drafts** | Discard any draft copy edits made in Journey 9 if they weren't real content changes |

`scripts/cleanup-test-submissions.mts` against staging handles (D1 + Sanity) in one shot. R2 is a separate CF-dashboard step.

**Don't skip this.** Test data on staging confuses the next round of testing and creates support-ticket-shaped objects in the reading-fulfillment queue.

---

## When done

- All 9 journeys passed → flag "smoke green" — production push cleared.
- Any journey failed → screenshot + brief note → flag the failing journey — that journey blocks production push until fixed.
- Cleanup was run → confirm — staging is back to a blank baseline.
