...
- **🚩 MUST FIX IN v1.18.0 (Max, 2026-07-10): `ccvoig0e` mobile-menu hamburger width jump.** Opening the hamburger removes the page scrollbar → fixed nav + content shift right by the scrollbar width. **Reopened in dex (was wrongly closed):** the "resolved by `scrollbar-gutter:stable`" closure was headless-Chromium-only (overlay/zero-width scrollbars report delta=0); Max still sees the jump with **classic scrollbars**, and #303 did NOT fix it. Needs real scrollbar-width compensation when overflow is locked, **verified against classic scrollbars, not headless Chromium**. This should land on `release/v1.18.0` before/with the #327 merge.
