...
## ▶ IN FLIGHT (2026-06-23 PM): booking funnel static (#312, ox2clqob) + R2 time-based ISR (#313, de287l3v) — BOTH MERGED to release/v1.11.5 + STAGING-DEPLOYED (CI green), awaiting Max RUNTIME smoke

- **🚨 OWED NOW (Max runtime smoke on staging, the real gate):** (a) edit a published Sanity doc, confirm the live (staging) page reflects it within ~60s (proves R2 time-based ISR actually revalidates on CF); (b) confirm `/book/[readingId]` + `/letter` nav is now fast (proves the incremental cache fixed the booking-funnel SSG runtime re-render). If either misbehaves, bailout = one-line `open-next.config.ts` revert (no migration to unwind). de287l3v dex left OPEN until this smoke passes.
- **#312 `ox2clqob` — MERGED (`eeab1eb`), dex DONE.** `/book/[readingId]` + `/letter` switched from the live draft fetchers to `publishedFetch` (3 new fetchers `fetchReadingPublished`/`fetchBookingPagePublished`/`fetchBookingFormPublished`). **Correction:** these pages were already `●` SSG in the build, NOT `ƒ`; #312 removes the runtime draft path. The actual booking-funnel SPEED fix is the incremental cache below (a `●` SSG page needs an incremental cache to be served from cache at runtime, else the Worker re-renders per request = the ~4s).
- **#313 de287l3v — MERGED (`01e975c`), dex OPEN pending smoke.** R2 **time-based ISR** (no tag cache, no DO, no PPR, no webhook). `open-next.config.ts` = `r2IncrementalCache` + `memoryQueue`; `wrangler.jsonc` staging gained a `WORKER_SELF_REFERENCE` service binding (R2 cache bucket `withjosephine-next-cache-staging` + `NEXT_INC_CACHE_R2_BUCKET` binding already existed inert from the reverted Stage 2); `publishedFetch` revalidate 300 to 60. Neither Stage 2 failure mode can recur (no `cacheComponents`/PPR `postponed state`; no `doShardedTagCache` read-storm). CI green incl deploy-staging + sanity-validate-staging. PRD `MEMORY/WORK/20260623-173338_r2-incremental-isr/PRD.md`.
