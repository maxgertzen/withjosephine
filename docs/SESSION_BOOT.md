...
**Most-recent merge:** PR #165 (`3166faf`) — smoke-walk Sub-PR ε (gift-recipient-listen sandbox roundtrip). **6 PRs merged on 2026-05-21** closing the 2026-05-20 staging smoke walk: #160 (β — schema drift S1+S2 + listen title U1), #161 (δ — recipientNameFor fallback C4 thank-you), #162 (α — notifyPaid guard C2 + partial C3), #164 (hotfix — δ regression on undefined email), #163 (γ — C1+C5 silent-400 instrumentation), #165 (ε — gift-recipient-listen e2e regression net). Staging deployed and verified. **Hold-gate item #6 (smoke walkthrough) now closed via fix arc.** Source PRD: `MEMORY/WORK/20260520-181127_smoke-walk-tdd-fix-and-ship/PRD.md`. Source handoff: `MEMORY/WORK/20260521-smoke-walk/HANDOFF.md`.
