...
- 🟠 **PR #198 OPEN AS DRAFT — kf1nixad PR-2 (`x2lhyv0j`)** on branch `feat/preview-replan-pr2-implementation`, base `release/v1.2.1`. Studio-bundled gated-page preview across **6 surfaces** (Listen / MyReadings / MyGifts / Gift Claim / Magic Link Verify / Gift Intake), **22 fixture states**. Tailwind compile pipeline emits 55KB CSS at studio:dev/build/deploy. `next/navigation` + `next/image` stubs at `studio/preview-stubs/` (the latter swaps relative-path imgs for a transparent SVG data URL — see follow-up `h1zu9nkr` for the CORP-same-origin background). Iframe `key={`${surface}-${stateKey}-${renderTick}`}` remounts on every render so Sanity edits don't go stale. Pre-push gates all green (lint 0 errors, typecheck clean, 1771/1771 tests, both builds ✓). **NEXT-SESSION ACTIONS**: (1) Max verifies all 6 preview surfaces locally — refresh `localhost:3333` after restarting `pnpm studio:dev`; state switcher works; copy edits live-update. (2) Mark PR #198 ready + squash-merge. (3) Bookkeeping: `dex complete x2lhyv0j` + `dex complete kf1nixad` + SESSION_BOOT update. (4) `pnpm studio:deploy` to push new bundle to `withjosephine.sanity.studio` (CI does NOT deploy Studio — manual step). (5) Real-browser smoke against deployed Studio. PRD: `MEMORY/WORK/20260524-172503_gated-page-preview-replan/PRD.md`. dex `x2lhyv0j` carries the full handoff.
- 🟡 `k14jh76o` — Post-deploy QATester smoke of email-preview iframes in deployed Studio.
- 🟡 `o1hejeb1` — Day7OverdueAlert visual review (admin-facing — may stay bare).
