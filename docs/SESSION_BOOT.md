...
- **Phase 3 `21jupako` — IN PROGRESS on branch `feat/path-a-phase3-static-cutover` (off `release/v1.11.5`). PRD: `MEMORY/WORK/20260622-140253_path-a-phase3-static-cutover/PRD.md` (19/44).**
  ...
  - **REMAINING = one coherent breaking flip (steps 2-6 below), best done fresh-context.** The consent-cookie + AnalyticsBootstrap rewiring is COUPLED to the layout strip (can't land additively), so steps 2-6 go together as one change. **CSP decision LOCKED (Max 2026-06-22): per-route — keep strict nonce CSP on dynamic routes; static routes get a nonce-free CSP admitting Next's inline bootstrap via build-time sha256 hash(es) captured EMPIRICALLY from the first static build; NO `experimental.sri`, NO `unsafe-inline`; JSON-LD nonce dropped (data block, exempt from script-src, research-confirmed).** Then `assert-static-routes.mjs` flips `/`,`/privacy`,`/terms`,`/refund-policy` FAIL→PASS. Then Chunk 4: tests + `/simplify` + `/code-review` pre-push → PR.
  - The 7 ordered steps below remain authoritative for the flip; step 1 is now half-done (preview routes built; layout-side live-tree move is part of step 2).
