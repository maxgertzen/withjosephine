...
## ▶ NEXT SESSION (2026-06-22 cont.): Stage 2 (cacheComponents) IMPLEMENTED — PR #307 draft on `release/v1.11.5`, gated on Sanity webhook + Max staging verify

**Stage 2 is built + committed + pushed** (`perf/stage2-cachecomponents`, PR **#307** draft → release/v1.11.5). prerender-manifest **2 → 24 content routes** (home, /terms, /privacy, /refund-policy, /book/*, /book/*/letter, /thank-you/* now Partial Prerender). typecheck + lint + **2443 tests** green; `next build` green. PRD `MEMORY/WORK/20260622-145010_stage2-cachecomponents/`.

- **What landed:** `cacheComponents:true` + `cacheLife:{default: sanity}` (next-sanity v13 applies the 1y `sanity` profile internally + tags with syncTags — it is **on-demand-by-design**, NOT time-based, confirmed in `node_modules/next-sanity/.../next-js/index.js:51`); central `sanityFetch` is `'use cache'` + a coarse `sanity:content` tag; root/authed layouts → static shells + Suspense islands (`SiteAnalytics`, `DraftModeChrome`, `AuthedUserMenu`); request-data pages wrapped in Suspense; `open-next.config` R2 incremental cache + DO sharded tag cache + `queue:"direct"` + `enableCacheInterception:false`.
- **Revalidation = reuse the EXISTING webhook (Sanity caps the free plan at 2 webhooks, both used).** `revalidateTag(SANITY_CONTENT_TAG,"max")` is folded into `/api/sanity-sync` (the "Content Update Event" webhook, fires on prod-dataset publish → **staging worker**). So a publish refreshes the **staging** Next cache. **No standalone revalidate route, no new webhook, no new secret.**
- **Binding-constraint reversal (Max-approved):** old "no incremental cache by design" was premised on all-routes-dynamic; Stage 2 inverts it. R2 cache buckets **`withjosephine-next-cache` + `-staging` CREATED**. wrangler (prod+staging): `NEXT_INC_CACHE_R2_BUCKET` + DO `NEXT_TAG_CACHE_DO_SHARDED` (migration v2). Add a CLAUDE.md binding-constraint line at v1.11.5 merge-to-main.
- **🚨 OWED before merge:** (1) **Max staging real-browser verify:** EU consent · Presentation overlay + edit handles + **live-update (may now self-fix — re-check `yqkukzlt`)** · nav speed vs Stage 0 baseline · authed pages · **publish→revalidation test** (edit in Studio, confirm staging page refreshes). (2) deploy-staging runs cf:build on merge (3 MiB cap verified there); buckets exist so the binding won't red. **No Sanity dashboard action needed.**
- **⚠️ Production revalidation gap (deferred to apex-unpark, epic `wdpz1ux4`):** the reused webhook points at the **staging** worker, so once apex unparks, production (`withjosephine.com`) content won't revalidate on publish until a webhook slot is sorted (retire/repoint one of the 2, or upgrade Sanity plan). Until unpark, production is parked — non-issue.
- **New global enforcement (this session):** PostToolUse warning hook for added code comments (`~/.claude/hooks/warn-added-comments.mjs`); memory `feedback_audit_before_adding_resources` (reuse before adding secrets/vars/bindings).
