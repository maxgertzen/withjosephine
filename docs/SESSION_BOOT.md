...
## ▶ NEXT SESSION (2026-07-10): v1.18.0 SHIPPED to prod — PR #327 merged (`78e9558`), tagged `v1.18.0`, deploy-production + smoke-production green. Next = post-deploy prod verification (Clarity real session; live mobile-menu on classic scrollbars)

**✅ PR #327 MERGED to `main` (2026-07-10, squash `78e9558`), tagged `v1.18.0`, deployed to prod** (deploy-production + smoke-production both green, run `29079026622`). All PR CI was green pre-merge: Playwright (chromium) incl. the new `nav-menu-no-shift` spec, test, lint-and-typecheck, storybook, osv-scan, deploy-staging, GROQ contract, sanity-validate-staging. Max authorized merge-on-green and had visually verified the nav fix. The v1.18.0 bundle is now live on the apex: perf, a11y, CSP hardening, privacy-copy, shared token refactor, SEO (#324/#325), Clarity (#328), and the mobile-menu scrollbar fix (`ccvoig0e`).
- **✅ CLARITY RECORDING FIXED + merged into `release/v1.18.0` (#328, `e540fc3`, 2026-07-10).** Clarity had recorded **zero** sessions since install: the hand-rolled bare-tag `ClarityScript` dropped the `window.clarity` queue stub → tag crashed (`a[c] is not a function`) → `scripts.clarity.ms` recording engine never loaded. Switched to the official `@microsoft/clarity` package (`Clarity.init` stages the stub CSP-safely; init+consent moved into `ClarityScript`'s effect, race gone), + `c.bing.com` added to `img-src`. Browser-verified locally (engine loads, `j.clarity.ms/collect` uploads). #327 re-ran green **with** this fix included. **Prod end-to-end (a session appearing in Clarity) confirms only post-`v1.18.0` deploy.** #327 pre-merge smoke should re-run since the CSP `img-src`/Clarity changed — folds into the existing "beacon-console-clear" smoke item. Clarity-Bot vs CF Bot-Fight-Mode = conditional dex `ahmw35sr` (only if playbacks render unstyled).
- **✅ FIXED `ccvoig0e` mobile-menu hamburger width jump (2026-07-10, direct commit on `release/v1.18.0`).** `useLockBodyScroll` now measures the real width the browser reclaims when the scrollbar is removed (`clientWidth` before vs after locking): overlay scrollbars → 0 (no-op, matches headless delta=0), classic scrollbars → their width, compensated via `padding-right` on `<html>` + published as `--j-scroll-lock-gutter`; the fixed nav + overlay offset their right edge by that var. `globals.css` untouched (measurement adapts to `scrollbar-gutter` either way). New mock e2e `nav-menu-no-shift` (guards the regression on classic-scrollbar CI). Verified visually by Max (menu items hold). #303's `scrollbar-gutter:stable` superseded.
- **🚩 START HERE — post-deploy prod verification (Max-actions, now that v1.18.0 is live).** Confirm Clarity records a real session end-to-end (`j.clarity.ms/collect` upload, a session appears in the dashboard); eyeball the live mobile-menu open on a **classic-scrollbar** browser (headless can't reproduce the width jump — Max already visually verified locally). The planned pre-merge staging smoke was NOT run (merged on green CI per Max authorization); if desired, spot-check on live prod: nav underline/border, skip-link Tab, FAQ expand+collapse, reduced-motion, `/_next/static` immutable `Cache-Control`, **beacon-console-clear** (`static.cloudflareinsights.com` loads, no CSP console error), **mobile-overlay Tab reaches the close button**.
