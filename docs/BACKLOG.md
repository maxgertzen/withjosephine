...
Pentester cross-phase re-audit on `main..release/v1.0.0` returned CONDITIONAL GO. H-1 + M-1 + M-2 + M-3 shipped in-PR (`feat/phase-5-audit-fixes`). Three LOWs deferred with explicit triggers:

- **L-1 — `assert-staging-bindings.mts` should also assert `NEXT_PUBLIC_SANITY_DATASET === "staging"`.** Today the asserter covers D1 + R2 + ENVIRONMENT + RESEND_DRY_RUN but not the dataset binding. (Note: Phase 5 PR adds the assertion already as part of M-1; this entry stands only if a regression strips it.) **Trigger to revisit:** any future PR removes the `vars.NEXT_PUBLIC_SANITY_DATASET` check from the asserter — promote back to MED.
...
- **L-3 — Drop or HMAC-sign the `?purchaserFirstName=` URL override on `/thank-you/[id]`.** Renders attacker-supplied text into greeting. React auto-escapes; `robots: noindex`; real risk is social-share phishing. (The sibling `?recipient=` override on `/gift/claim/[token]` already shipped a fix in Phase 5 PR — M-3 dropped it.) **Trigger to promote:** any Mixpanel/Sentry event shows the param arriving from an inbound link that wasn't generated by our own redirect chain.
